...
class VeiculoDAO:
    ...
    def obter_veiculo_por_id(self, veiculo_id):
    ...
    def atualizar_veiculo(self, veiculo):
        ...
        try:
            ...
            WHERE id = %s
            """
            values = (veiculo.cliente_id, veiculo.marca, veiculo.modelo, 
                     veiculo.matricula, veiculo.pais, veiculo.id)
    ...
    def verificar_matricula_existe(self, matricula, veiculo_id=None):
        """Verifica se uma matrícula já existe (excluindo um veículo específico se fornecido)"""
        try:
            conn = get_db_connection()
            cursor = conn.cursor()
